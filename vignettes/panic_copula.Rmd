...
Assume there is a equally-weighted portfolio of stocks of the indexes that appears in object `x`.
...
To address this question we follow [*A New Breed for Copulas for Risk and Portfolio Management*](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=1752702) and model the market as a mixture of "calm" vs. "panic" distributions. For details on the full specification of this market, please, see the reference attached.
...
In the first scenario we assume there is a `2%` probability of panic, in which *all* the cross-correlations are equal to `0.97`:
...
The new marginal distribution shows a hump shape format around `-2%`, which is a direct consequence of the panic we just modeled. Nevertheless, the location and dispersion parameters still matches the sample counterparts. *The panic is hidden*!

The full picture - with the similarities and differences - among these markets regiems can be seen with help of `ggplot2`:
...
While the location and dispersion doesn't change, all the other metrics are twisted for markets that operate under dual regimes. The objects `stats_panic` and `stats_calm` imply that a 2% probability of panic increases the kurtosis by 6% and VaR and CVaR by 10%.
